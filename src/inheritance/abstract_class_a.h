...
But explicitly overriding ~AbstractClassA() is illegal
in child class, so counterintuitively, you will need
to provide definition of destructor in a separate
AbstractClassA.cpp file which overcomplicates the matter.
You might just define it in the header.
*/
#ifndef SRC_INHERITANCE_ABSTRACT_CLASS_A_H_
#define SRC_INHERITANCE_ABSTRACT_CLASS_A_H_
...
class AbstractClassA {
 public:  // all interface methods should be public
    ...
    virtual ~AbstractClassA() {
        std::cout << "~AbstractClassA" << std::endl;
    }

    // all interface methods must be marked pure (=0)
    // so that the compiler will check they are implemented
    // by child class
    virtual std::string InterfaceMethodA(const std::string& arg) const = 0; 

    // do not declare attributes in interface
};
}  // namespace cpp_boilerplate
#endif  // SRC_INHERITANCE_ABSTRACT_CLASS_A_H_
